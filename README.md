...
Значения $\beta$ и связности пиксельного графа задаются в интерфейсе приложения в панели настроек алгоритма. Параметры хранятся в настройках приложения и передаются в математическое ядро через:
...
- `model/graph/GridLaplacian.hpp/cpp` — построение взвешенного графа с выбранной связностью и переданным значением $\beta$.
...
При выборе 8-связности меняется только множество соседей grid-графа. Весовая функция ребра остается той же; диагональные ребра не получают дополнительной геометрической нормировки.
