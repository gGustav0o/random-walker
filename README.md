...
Значения $\beta$, связности пиксельного графа и степени нормировки по расстоянию $p$ задаются в интерфейсе приложения в панели настроек алгоритма. Параметры хранятся в настройках приложения и передаются в математическое ядро через:
...
- `model/graph/GridLaplacian.hpp/cpp` — построение взвешенного графа с выбранной связностью, $\beta$ и степенью расстояния $p$.
...
При `p = 0` поведение совпадает с классической формулой без геометрической нормировки. При `p > 0` диагональные ребра в 8-связности ослабляются пропорционально `sqrt(2)^p`, тогда как ортогональные ребра не меняются.
