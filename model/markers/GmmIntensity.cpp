...
namespace random_walker::markers {
    namespace {
        ...
        [[nodiscard]] bool has_valid_components(
        ...
        [[nodiscard]] double gaussian_density(
            ...
            const double exponent =
                -difference * difference / (2.0 * component.variance);
        ...
        [[nodiscard]] std::size_t foreground_component_index(
            const GmmModel1d& model
            ...
            const auto iterator =
                polarity == domain::ForegroundPolarity::DarkObject
        ...
        [[nodiscard]] std::vector<WeightedIntensitySample>
        ...
        [[nodiscard]] GmmModel1d initialize_gmm(
            std::span<const WeightedIntensitySample> samples
    ...
    double foreground_probability(
        ...
        const std::array<double, domain::kGmmIntensityComponentCount>
            posterior = posterior_probabilities(model, value);
        return posterior[foreground_component_index(model, polarity)];
    }

    GmmFitOutcome fit_gmm(
        std::span<const WeightedIntensitySample> samples
        , const domain::GmmParameters& parameters
    ) {
