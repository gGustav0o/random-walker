...
namespace random_walker::markers {
    namespace {
        ...
        [[nodiscard]] int image_pixel_count(
        ...
        [[nodiscard]] IntensityHistogram image_intensity_histogram(
            const domain::GrayImage& image
        ) noexcept {
            assert(!image.empty());
        ...
        [[nodiscard]] std::vector<CandidatePixel> high_confidence_candidates(
            const domain::GrayImage& image
        ...
        [[nodiscard]] std::vector<std::uint8_t> distance_safe_candidates(
            ...
            for (const domain::SeedLabel label : {
                ...
                std::vector<std::uint8_t> complement_mask(
                    accepted_by_label.size()
                    , 0U
                );
        ...
        void append_clean_component_markers(
            ...
            if (component_indices.size()
                ...
                diagnostics.rejected_small_component_count +=
        ...
        [[nodiscard]] domain::MarkerLabelMask clean_candidates(
            const std::vector<CandidatePixel>& candidates
            , const domain::GrayImage& image
            , const domain::AutoMarkerParameters& parameters
            , domain::MarkerProposalDiagnostics& diagnostics
            ...
            const std::size_t pixel_count = domain::pixel_count(width, height);
            ...
            const std::vector<std::uint8_t> candidate_is_safe =
                distance_safe_candidates(
                    candidates
                    , accepted_by_label
                    , width
                    , height
            ...
            for (std::size_t start_index = 0;
                ...
                while (!queue.empty()) {
                    ...
                    for (int offset_index = 0;
                        ...
                        const int neighbor_pixel_index =
                            flatten(neighbor, width);
                        const int neighbor_candidate_index =
                            candidate_index_by_pixel[
        ...
        [[nodiscard]] bool contains_manual_seed(
            domain::PixelCoordinate position
    ...
    domain::MarkerProposal remove_manual_conflicts(
    ...
    MarkerProposalOutcome propose_gmm_markers(
        const domain::GrayImage& image
        , const domain::AutoMarkerParameters& parameters
    ) {
        if (image.empty()) {
