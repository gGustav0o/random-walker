...
namespace random_walker::algorithm {
    ...
    [[nodiscard]] inline std::vector<double> squared_distance_transform_1d(
        std::span<const double> samples
    ) {
        const int count = static_cast<int>(samples.size());
        std::vector<double> distances(samples.size(), std::numeric_limits<double>::infinity());
        if (count == 0) {
            return distances;
        }
        ...
        for (int sample_index = parabola_indices[0] + 1;
            ...
            const auto intersection = [&samples](int left, int right) {
                ...
                return (
                    samples[static_cast<std::size_t>(right)] + right_position * right_position
                    - samples[static_cast<std::size_t>(left)] - left_position * left_position
            ...
            while (crossing <= breakpoints[static_cast<std::size_t>(envelope_index)]) {
                --envelope_index;
            ...
            ++envelope_index;
            parabola_indices[static_cast<std::size_t>(envelope_index)] = sample_index;
        ...
        for (int sample_index = 0; sample_index < count; ++sample_index) {
            while (breakpoints[static_cast<std::size_t>(envelope_index + 1)]
            ...
            const int nearest_index =
                parabola_indices[static_cast<std::size_t>(envelope_index)];
            const double offset = static_cast<double>(sample_index - nearest_index);
    ...
    [[nodiscard]] inline std::vector<double> squared_euclidean_distance_transform(
        int width
        , int height
        ...
        const std::size_t pixel_count =
            static_cast<std::size_t>(width) * static_cast<std::size_t>(height);
        std::vector<double> column_pass(pixel_count, std::numeric_limits<double>::infinity());
        std::vector<double> distances(pixel_count, std::numeric_limits<double>::infinity());
        if (width == 0 || height == 0) {
            return distances;
        }

        std::vector<double> line(static_cast<std::size_t>(std::max(width, height)), 0.0);
        for (int column = 0; column < width; ++column) {
            ...
            const std::vector<double> transformed = squared_distance_transform_1d(
                std::span<const double>(line.data(), static_cast<std::size_t>(height))
            );
            for (int row = 0; row < height; ++row) {
                column_pass[
                    static_cast<std::size_t>(row) * static_cast<std::size_t>(width)
        ...
        for (int row = 0; row < height; ++row) {
            ...
            const std::vector<double> transformed = squared_distance_transform_1d(
                std::span<const double>(line.data(), static_cast<std::size_t>(width))
            );
            for (int column = 0; column < width; ++column) {
                distances[
                    static_cast<std::size_t>(row) * static_cast<std::size_t>(width)
